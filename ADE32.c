...
int disasm(uint8_t* opcode0, disasm_struct* diza)
{
    ...
    if(*(uint16_t*)opcode == 0x0000)
        return 0;
    if(*(uint16_t*)opcode == 0xFFFF)
        return 0;
...
//		goto repeat_prefix;
    ...
    int a =  flag & (C_ADDR1 | C_ADDR2 | C_ADDR4);
    int d = (flag & (C_DATA1 | C_DATA2 | C_DATA4)) >> 8;
